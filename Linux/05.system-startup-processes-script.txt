...
if [ $fware == 'BIOS' ]; then
	if [ $distro == 'RHEL' ]; then
		grep terminal_output /boot/grub2/grub.cfg -A 10 | grep timeout=60 &> /dev/null;
	elif [ $distro == 'SUSE' ]; then 
		grep terminal_output /boot/grub2/grub.cfg -A 30 | grep timeout=60 &> /dev/null;
	elif [ $distro == 'Debian' ]; then
		grep terminal_output /boot/grub/grub.cfg -A 10 | grep timeout=60 &> /dev/null;
	fi
else
        if [ $distro == 'RHEL' ]; then
                grep terminal_output /boot/efi/EFI/centos/grub.cfg -A 10 | grep timeout=60 &> /dev/null;
        elif [ $distro == 'SUSE' ]; then
                grep terminal_output /boot/efi/EFI/opensuse/grub.cfg -A 10 | grep timeout=60 &> /dev/null;
        elif [ $distro == 'Debian' ]; then
                grep terminal_output /boot/efi/EFI/ubuntu/grub.cfg -A 10 | grep timeout=60 &> /dev/null;
        fi
fi
if [ $? -eq 0 ]; then tr='PASS'; else tr='ERROR'; fi
echo '... '$tr
